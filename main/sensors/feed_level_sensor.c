...
static const char *TAG = "FEED_LEVEL";

#define DEFAULT_VREF    1100        // Default reference voltage in mV
#define NO_OF_SAMPLES   64          // Multisampling

static feed_level_config_t sensor_config = {0};
...
esp_err_t feed_level_sensor_init(const feed_level_config_t *config) {
    ...
    memcpy(&sensor_config, config, sizeof(feed_level_config_t));
    ...
    if (sensor_config.hopper_capacity_kg == 0) {
        sensor_config.hopper_capacity_kg = 50.0f; // Default 50kg hopper
    }
    ...
    initialized = true;
    ESP_LOGI(TAG, "Feed level sensor initialized on ADC channel %d", sensor_config.adc_channel);
...
esp_err_t feed_level_sensor_read(float *percent) {
    ...
    // Convert voltage to percentage (load cell/ultrasonic sensor characteristic)
...
esp_err_t feed_level_sensor_read_kg(float *kg) {
    if (!initialized || kg == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    
    float percent = 0;
    esp_err_t ret = feed_level_sensor_read(&percent);
    if (ret != ESP_OK) {
        return ret;
    }
    
    // Convert percentage to kilograms
    *kg = (percent / 100.0f) * sensor_config.hopper_capacity_kg;
...
esp_err_t feed_level_sensor_calibrate(float known_percent) {
    ...
    if (feed_level_sensor_read(&current_percent) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t feed_level_sensor_deinit(void) {
    ...
    ESP_LOGI(TAG, "Feed level sensor deinitialized");
    return ESP_OK;
}
